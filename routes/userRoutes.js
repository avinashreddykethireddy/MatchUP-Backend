...
router.get('/recentProducts/:userId', async (req, res) => {
    ...
    User.findOne({ _id: userId}, function(err, user) {
        if(err) {
            ...
            return res.status(200).json({ recentProducts: user.recentlyViewedProducts || [] });
        }
    })
});
...
router.delete('/recentProducts/:userId/:productId', async (req, res) => {
    ...
    try {
        ...
        if(products.length !== 0){
            products.forEach(ele => {
                if(req.params.productId.toString() === ele._id.toString()){
                    products.remove(ele);
                }
            })
        }
...
router.post('/favouriteBlogs/:userId/:blogId', async (req, res) => {
    ...
    User.findOne({_id: userId}, function(err, user) {
        if(err) {
            ...
            if(blogs.length !== 0){
                blogs.forEach(ele => {
                    if(currentBlog._id.toString() === ele._id.toString()){
                        flag = 0;
                    }
                });
            }
            if(flag){
                user.favouriteBlogs.push(currentBlog);
            }
...
router.delete('/favouriteBlogs/:userId/:blogId', async (req, res) => {
    ...
    try {
        ...
        if(blogs.length !== 0){
            blogs.forEach(ele => {
                if(req.params.blogId.toString() === ele._id.toString()){
                    blogs.remove(ele);
                }
            })
        }
    ...
    try {
        ...
        User.findOne({_id: userId}, async function(err, user) {
            if(err) {
                ...
                if(products.length !== 0){
                    products.forEach(ele => {
                        if(productId.toString() === ele._id.toString()){
                            products.remove(ele);
                        }
                    })
                }
